...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-dvh items-center justify-center bg-dark-950 px-4 py-10">
      <div className="w-full max-w-md rounded-none border border-dark-800/50 bg-dark-900/50 p-6 sm:p-8">
        {children}
      </div>
    </div>
  );
        ...
        >
          ...
          {result.subscription_url && (
            <>
              ...
              <p className="w-full select-all truncate rounded-none bg-dark-900/60 px-3 py-2 text-sm text-accent-400">
                {result.subscription_url}
              </p>
              <button
                type="button"
                ...
                className={cn(
                  'flex w-full items-center justify-center gap-2 rounded-none px-4 py-3 text-sm font-bold transition-all active:scale-[0.98]',
                  copied
  ...
  return (
    <Shell>
      ...
      >
        ...
        {gift.gift_message && (
          <div className="w-full rounded-none border border-dark-700/30 bg-dark-800/40 p-4 text-left">
            <p className="text-sm italic text-dark-200">&ldquo;{gift.gift_message}&rdquo;</p>
          </div>
        )}

        {willReplace && (
          <p className="w-full rounded-none border border-warning-500/20 bg-warning-500/5 p-3 text-xs text-warning-400">
            {t(
        ...
        {gift.bot_claim_link && (
          <a
            ...
            className="flex w-full items-center justify-center gap-2 rounded-none bg-accent-500 px-6 py-3.5 text-sm font-bold text-on-accent shadow-lg shadow-accent-500/25 transition-all hover:bg-accent-400 active:scale-[0.98]"
          >
        ...
        {!showEmail ? (
          <button
            ...
            className="w-full rounded-none border border-dark-700/50 px-6 py-3 text-sm font-medium text-dark-200 transition-colors hover:bg-dark-800/50"
          >
          ...
          <div className="w-full space-y-3 text-left">
            ...
            <input
              ...
              className="w-full rounded-none border border-dark-700/50 bg-dark-800/50 px-4 py-3 text-sm text-dark-50 placeholder-dark-500 outline-none focus:border-accent-500/50 focus:ring-1 focus:ring-accent-500/25"
            />
            ...
            <button
              ...
              className={cn(
                'flex w-full items-center justify-center gap-2 rounded-none px-6 py-3.5 text-sm font-bold transition-all',
