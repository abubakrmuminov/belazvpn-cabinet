...
export default function InstallationGuide({
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      <div className="flex items-center gap-3 border-b border-dark-300 pb-4">
        {!isTelegramWebApp && (
          <button
            onClick={onGoBack}
            aria-label={t('common.back', 'Back')}
            className="flex h-10 w-10 items-center justify-center border border-dark-300 bg-dark-800 shadow-[2px_2px_0_0_#000] transition-all hover:bg-dark-700 active:translate-y-[1px] active:shadow-[1px_1px_0_0_#000]"
          >
            <BackIcon className="h-6 w-6" />
          </button>
        )}
        <h2 className="flex-1 font-mono text-base font-black uppercase tracking-widest text-dark-100">
        ...
        {appConfig.subscriptionUrl && onOpenQR && (
          <button
            ...
            className="flex h-10 w-10 items-center justify-center border border-dark-300 bg-dark-800 text-dark-200 shadow-[2px_2px_0_0_#000] transition-all hover:bg-dark-700 active:translate-y-[1px] active:shadow-[1px_1px_0_0_#000]"
          >
            <svg
              className="h-5 w-5"
              fill="none"
        ...
        {availablePlatforms.length > 1 && (
          <div className="relative flex items-center">
            ...
            <select
              ...
              className={`appearance-none border bg-dark-800 py-2 pr-8 font-mono text-xs font-bold uppercase tracking-wider text-dark-200 outline-none shadow-[2px_2px_0_0_#000] transition-all hover:bg-dark-700 border-dark-300 ${
          ...
          {currentPlatformApps.map((app, idx) => {
            ...
            return (
              <button
                ...
                className={`relative flex min-w-[calc(50%-0.25rem)] items-center gap-2 overflow-hidden border px-4 py-2 font-mono text-xs font-black uppercase tracking-wider shadow-[2px_2px_0_0_#000] transition-all active:translate-y-[1px] active:shadow-[1px_1px_0_0_#000] ${
                  isSelected
                    ? 'border-accent-500 bg-accent-500/15 text-accent-400'
                    : 'border-dark-300 bg-dark-800 text-dark-200 hover:border-dark-200 hover:bg-dark-700'
                }`}
              >
                {app.featured && <span className="h-2 w-2 shrink-0 bg-warning-400" />}
      ...
      {appConfig.baseSettings?.isShowTutorialButton && appConfig.baseSettings?.tutorialUrl && (
        <button
          type="button"
          onClick={() => openLink(appConfig.baseSettings?.tutorialUrl ?? '')}
