...
export default function TvQuickConnect({ subscriptionUrl, isLight }: Props) {
  ...
  const inputClass = isLight
    ? 'w-full rounded-none border border-dark-700/60 bg-white px-4 py-3 text-center text-2xl font-bold tracking-[0.3em] uppercase text-dark-100 outline-none focus:border-accent-500 focus:ring-1 focus:ring-accent-500'
    : 'w-full rounded-none border border-dark-700 bg-dark-900/50 px-4 py-3 text-center text-2xl font-bold tracking-[0.3em] uppercase text-dark-100 outline-none focus:border-accent-500 focus:ring-1 focus:ring-accent-500';
    ...
    <div className="mt-3 space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className={scanning ? 'space-y-2' : 'hidden'}>
          <div id="tv-qr-reader" className="overflow-hidden rounded-none" />
          {scanning && (
      ...
      {toast && (
        <div
          // На мобильном стоит над нижней панелью
          className={`fixed bottom-[var(--mobile-nav-clearance)] left-1/2 z-50 -translate-x-1/2 rounded-none px-5 py-3 text-sm font-medium shadow-lg transition-all lg:bottom-6 ${
