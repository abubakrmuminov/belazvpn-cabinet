  ...
  return createPortal(
    ...
    >
      <div
        ...
        tabIndex={-1}
        className="w-full max-w-md rounded-none bg-[#17212b] p-4 shadow-2xl"
      ...
      >
        ...
        <div className="rounded-none bg-[#0e1621] p-3">
          <div className="ml-auto max-w-[90%] rounded-none rounded-tr-md bg-[#2b5278] p-3 text-white shadow">
            {mediaUrl && mediaType === 'photo' && (
              <img
                src={mediaUrl}
                alt=""
                loading="lazy"
                className="mb-2 max-h-72 w-full rounded-none object-cover"
              />
            )}
            {mediaUrl && mediaType === 'video' && (
              <video src={mediaUrl} controls className="mb-2 max-h-72 w-full rounded-none" />
            )}
            {text ? (
                ...
                <div key={ri} className="flex gap-1">
                  {row.map((b, ci) => (
                    <button
                      key={ci}
                      type="button"
                      className="flex-1 rounded-none bg-[#1f2c3a] px-3 py-2 text-sm text-blue-300 hover:bg-[#26384a]"
                    >
                      {b.text}
                    </button>
                  ))}
  ...
  return createPortal(
    ...
    >
      <div
        ...
        tabIndex={-1}
        className="flex h-[80vh] w-full max-w-2xl flex-col rounded-none bg-white shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
