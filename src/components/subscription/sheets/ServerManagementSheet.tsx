...
export function ServerManagementSheet({
  ...
  if (!open) {
    return (
      <button
        onClick={onOpen}
        className={`w-full rounded-none border p-4 text-left transition-colors ${isDark ? 'border-dark-700/50 bg-dark-800/50 hover:border-dark-600' : 'border-champagne-300/60 bg-champagne-200/40 hover:border-champagne-400'}`}
      >
  ...
  return (
    <div
      className={`rounded-none border p-5 ${isDark ? 'border-dark-700/50 bg-dark-800/50' : 'border-champagne-300/60 bg-champagne-200/40'}`}
    >
      ...
      {countriesLoading ? (
        ...
        <div className="space-y-4">
          <div
            className={`rounded-none p-2 text-xs ${isDark ? 'bg-dark-700/30 text-dark-500' : 'bg-champagne-300/40 text-champagne-600'}`}
          >
            {t('subscription.serverManagement.statusLegend')}
          </div>

          {countriesData.discount_percent > 0 && (
            <div className="rounded-none border border-success-500/30 bg-success-500/10 p-2 text-xs text-success-400">
              🎁{' '}
              ...
              .map((country) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-none border p-3 text-left transition-all ${
                      isSelected
                        ? willBeAdded
